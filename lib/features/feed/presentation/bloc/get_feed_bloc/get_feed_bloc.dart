...
class GetFeedBloc extends Bloc<GetFeedEvents, GetFeedStates> {
  GetFeedBloc() : super(GetFeedInitialState()) {
    on<GetFeedData>((event, emit) async {
      ...
      final (postsSuccess, posts, postsMessage) = await sl<GetFeedPostsUsecase>()
          .getFollowingPosts(tokenId!);
      ...
      if (postsSuccess && storiesSuccess) {
        emit(
          GetFeedSuccessState(
            title: "Feed Loaded",
      ...
      emit(GetFeedErrorState(
        ...
        description:
            postsMessage ?? storiesMessage ?? "An unknown error occurred.",
      ));
    });
  }
}
